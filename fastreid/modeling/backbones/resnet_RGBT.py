...
class ResNet(nn.Module):
    def __init__(self, last_stride, bn_norm, with_ibn, with_se, with_nl, block, layers, non_layers, RGBT_mode):
        self.inplanes = 64
        super().__init__()

        self.RGBT_mode = RGBT_mode
        if RGBT_mode == 'Concat':
...
def build_resnet_RGBT_Concat_backbone(cfg):
    """
    ...
    if pretrain:
        ...
        weight_init = state_dict['conv1.weight']
        state_dict['conv1.weight'] = torch.nn.Parameter(
            torch.cat((weight_init, weight_init.mean(1, keepdim=True)), dim=1))
...
@BACKBONE_REGISTRY.register()
def build_resnet_RGBT_Feature_backbone(cfg):
    """
    Create a ResNet instance from config.
    Returns:
        ResNet: a :class:`ResNet` instance.
    ...
    if pretrain:
        ...
        weight_init = state_dict['conv1.weight']
        state_dict['conv1_t.weight'] = torch.nn.Parameter(weight_init.mean(1, keepdim=True))
