...
def gen_reid_data(scenarios,
    ...
    for s_id in scenarios: # [s01, s10, ..., s47]
        ...
        for modality in ['rgb', 'thermal']:
            if osp.exists(osp.join(out_dir, modality)):
                shutil.rmtree(osp.join(out_dir, modality))
            os.makedirs(osp.join(out_dir, modality))
        ...
        for csv_name in csv_names: # ['s01_c01', s01_c02', ..., 's01_c16']
            ...
            for f_id in f_ids:
                ...
                thermal_path = osp.join(frames_root, s_id, c_id, 'thermal', img_name)
                ...
                for idx, ann in f_anns.iterrows():
                    ...
                    crop_thermal = thermal.crop((x1, y1, x2, y2))
						
                    out_img_path = osp.join(out_dir, 'rgb', out_img_name)
                    out_thermal_path = osp.join(out_dir, 'thermal', out_img_name)
                    
                    crop_img.save(out_img_path)
                    crop_thermal.save(out_thermal_path)

def move_imgs(imgs, s_root, t_root):
    for modality in ['rgb', 'thermal']:
        for img in imgs:
            s_path = osp.join(s_root, modality, img)
            t_path = osp.join(t_root, modality, img)
...
def split_reid_data(scenarios, out_root):
    ...
    for modality in ['rgb', 'thermal']:
        if osp.exists(osp.join(train_img_root, modality)):
            shutil.rmtree(osp.join(train_img_root, modality))
        if osp.exists(osp.join(query_img_root, modality)):
            shutil.rmtree(osp.join(query_img_root, modality))
        if osp.exists(osp.join(gallery_img_root, modality)):
            shutil.rmtree(osp.join(gallery_img_root, modality))
        os.makedirs(osp.join(train_img_root, modality))
        os.makedirs(osp.join(query_img_root, modality))
        os.makedirs(osp.join(gallery_img_root, modality))

    pattern = re.compile(r'([\d]+)_c([\d]+)')

    for s_id in scenarios:
        reid_img_root = osp.join(out_root, s_id)
        reid_imgs = os.listdir(osp.join(reid_img_root, 'rgb'))
    ...
    rename_train_imgs(osp.join(train_img_root, 'rgb'))
    rename_train_imgs(osp.join(train_img_root, 'thermal'))
    rename_test_imgs(osp.join(query_img_root, 'rgb'), osp.join(gallery_img_root, 'rgb'))
    rename_test_imgs(osp.join(query_img_root, 'thermal'), osp.join(gallery_img_root, 'thermal'))
